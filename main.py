...
app.add_middleware(SessionMiddleware, secret_key=SESSION_SECRET, session_cookie="gapino_session", max_age=AUTH_TOKEN_TTL, same_site="none", https_only=True)
...
def make_access_token(user_id: int) -> str:
    payload = f"{int(user_id)}:{int(time.time()) + AUTH_TOKEN_TTL}"
    signature = hmac.new(
...
def bearer_user(request: Request) -> dict[str, Any] | None:
    ...
    return user_by_id(user_id) if user_id is not None else None
...
def register(request: Request, username: str = Form(...), password: str = Form(...), display_name: str = Form(...)):
    ...
    return {"ok": True, "success": True, "token": make_access_token(user_id), "user": public_user(user)}
...
def login(request: Request, username: str = Form(...), password: str = Form(...)):
    ...
    request.session["uid"] = user_id
    user = user_by_id(user_id)
    response = JSONResponse({"ok": True, "success": True, "token": make_access_token(user_id), "user": public_user(user)})
    response.set_cookie(
        "gapino_session",
...
def me(request: Request):
    ...
    response = JSONResponse({**public_user(user), "token": make_access_token(int(user["id"]))})
...
async def websocket_endpoint(websocket: WebSocket):
    ...
    try:
        ...
        while True:
            ...
            # -------------------------------------------------
            # WebRTC media stays peer-to-peer. The server only
            # forwards the signaling messages to the other user.
            if message_type in {
                "call_offer",
                "call_answer",
                "call_ice",
