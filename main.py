...
def verify_password(password: str, stored_hash: str) -> bool:
    ...
    if stored_hash.startswith("$pbkdf2$") and not stored_hash.startswith("$pbkdf2-sha256$"):
        ...
        try:
            ...
            return False
    try:
        return bool(pwd.verify(password, stored_hash))
    except Exception:
        pass
    for scheme_name, prefix in (("argon2", "$argon2"), ("bcrypt", ("$2a$", "$2b$", "$2y$"))):
        if (stored_hash.startswith(prefix) if isinstance(prefix, str) else stored_hash.startswith(prefix)):
            try:
                scheme = getattr(__import__("passlib.hash", fromlist=[scheme_name]), scheme_name)
                return bool(scheme.verify(password, stored_hash))
            except Exception:
                return False
    return False
...
def needs_password_upgrade(stored_hash: str) -> bool:
    ...
    return (
        ...
        and not value.startswith("$pbkdf2-sha256$")
    ) or value.startswith("$argon2") or value.startswith(("$2a$", "$2b$", "$2y$"))
...
def user_by_username(username: str) -> dict[str, Any] | None:
    conn = db()
    row = conn.execute(
        "SELECT id, username, display_name, bio, avatar, status, created_at FROM users WHERE LOWER(username) = LOWER(?)",
...
def current_user(request: Request) -> dict[str, Any] | None:
    ...
    try:
        return user_by_id(int(uid)) if uid is not None else None
...
def cleanup_tickets() -> None:
    ...
    for ticket in list(ws_tickets):
        if float(ws_tickets[ticket].get("expires", 0)) <= now_ts:
...
def register(request: Request, username: str = Form(...), password: str = Form(...), display_name: str = Form(...)):
    ...
    if not re.fullmatch(r"[a-zA-Z0-9_.-]+", username):
        raise HTTPException(status_code=400, detail="نام کاربری فقط می‌تواند شامل حروف انگلیسی، عدد، نقطه، خط تیره و زیرخط باشد")
    if len(password) < 6:
    ...
    try:
        cursor = conn.execute(
            ...
            (username, hash_password(password), display_name, now(), recovery_code),
        )
        conn.commit()
    ...
    response = JSONResponse({"ok": True, "success": True, "user": public_user(user), "recovery_code": recovery_code})
    set_ws_cookie(user_id, response)
    return response
...
def login(request: Request, username: str = Form(...), password: str = Form(...)):
    ...
    user_id = int(row["id"])
    if needs_password_upgrade(row["password"]):
        conn = db()
    ...
    request.session["uid"] = user_id
    user = user_by_id(user_id)
    response = JSONResponse({"ok": True, "success": True, "user": public_user(user)})
    set_ws_cookie(user_id, response)
    return response
...
def logout(request: Request):
    ...
    if uid is not None:
        try:
            sockets = connections.pop(int(uid), set())
            for websocket in list(sockets):
                try:
                    # connection will close naturally or by client
                    pass
                except Exception:
                    pass
        except (TypeError, ValueError):
    ...
    response.delete_cookie("gapino_session", path="/")
    response.delete_cookie("gapino_ws_ticket", path="/")
    return response
...
async def recover_password(request: Request):
    ...
    conn.execute("UPDATE users SET password=? WHERE id=?", (hash_password(new_password), int(row["id"])))
    conn.commit()
    conn.close()
...
def create_recovery_code(request: Request):
    user = require_user(request)
...
async def update_profile(request: Request):
    ...
    conn = db()
    conn.execute("UPDATE users SET display_name=?,bio=?,status=? WHERE id=?", (display_name, bio, status, int(user["id"])))
    conn.commit()
    conn.close()
...
async def update_account_security(request: Request):
    ...
    try:
        ...
        raise HTTPException(status_code=400, detail="داده امنیتی نامعتبر است")
    requested_username = data.get("username")
    current_password = data.get("current_password")
    new_password = data.get("new_password")
    username_changed = requested_username is not None and str(requested_username).strip().lower() != str(user["username"]).strip().lower()
    password_changed = new_password is not None and str(new_password) != ""
    if not username_changed and not password_changed:
        raise HTTPException(status_code=400, detail="تغییری برای ذخیره وجود ندارد")
    ...
    if password_changed:
        ...
        if not old_password:
            raise HTTPException(status_code=400, detail="رمز عبور فعلی را وارد کنید")
        if len(new_password_value) < 6:
            raise HTTPException(status_code=400, detail="رمز عبور جدید باید حداقل ۶ کاراکتر باشد")
        conn = db()
        row = conn.execute("SELECT password FROM users WHERE id=?", (int(user["id"]),)).fetchone()
        conn.close()
        ...
        hashed_password = hash_password(new_password_value)
    conn = db()
    try:
        if password_changed:
            conn.execute("UPDATE users SET username=?,password=? WHERE id=?", (new_username, hashed_password, int(user["id"])))
        else:
        ...
        conn.close()
    updated = user_by_id(int(user["id"]))
    return {"ok": True, "success": True, "message": "اطلاعات امنیتی با موفقیت تغییر کرد.", "user": public_user(updated)}
...
def users(request: Request):
    ...
    if query:
        pattern = f"%{query}%"
        rows = conn.execute(
            "SELECT id,username,display_name,bio,avatar,status,created_at FROM users WHERE id != ? AND (LOWER(username) LIKE ? OR LOWER(display_name) LIKE ?) ORDER BY display_name COLLATE NOCASE",
            (int(user["id"]), pattern, pattern),
        ).fetchall()
    else:
        rows = conn.execute(
            "SELECT id,username,display_name,bio,avatar,status,created_at FROM users WHERE id != ? ORDER BY display_name COLLATE NOCASE",
            (int(user["id"]),),
        ).fetchall()
    conn.close()
...
async def create_message(request: Request):
    ...
    conn = db()
    cursor = conn.execute("INSERT INTO messages(sender_id,receiver_id,text,created_at) VALUES(?,?,?,?)", (int(user["id"]), receiver_id, text, now()))
    conn.commit()
...
async def delete_message(request: Request, message_id: int):
    ...
    conn.execute("UPDATE messages SET deleted=1,text='',file_name='',file_url='' WHERE id=?", (message_id,))
    conn.commit()
    updated = conn.execute("SELECT * FROM messages WHERE id=?", (message_id,)).fetchone()
    conn.close()
    payload = dict(updated)
...
async def upload(request: Request, receiver_id: int = Form(...), file: UploadFile = File(...)):
    ...
    allowed = {".png", ".jpg", ".jpeg", ".webp", ".gif", ".pdf", ".txt", ".csv", ".zip", ".rar", ".mp3", ".wav", ".ogg", ".webm", ".mp4", ".mov", ".doc", ".docx", ".xls", ".xlsx", ".ppt", ".pptx"}
    ...
    conn = db()
    cursor = conn.execute(
        "INSERT INTO messages(sender_id,receiver_id,text,file_name,file_url,mime_type,created_at) VALUES(?,?,?,?,?,?,?)",
...
async def create_group(request: Request):
    ...
    conn = db()
    cursor = conn.execute("INSERT INTO groups(name,owner_id,created_at) VALUES(?,?,?)", (name, int(user["id"]), now()))
    group_id = int(cursor.lastrowid)
    conn.execute("INSERT OR IGNORE INTO group_members(group_id,user_id) VALUES(?,?)", (group_id, int(user["id"])))
    for member_id in members:
        if conn.execute("SELECT id FROM users WHERE id=?", (member_id,)).fetchone():
            conn.execute("INSERT OR IGNORE INTO group_members(group_id,user_id) VALUES(?,?)", (group_id, member_id))
    conn.commit()
    conn.close()
...
async def websocket_endpoint(websocket: WebSocket):
    ...
    try:
        await websocket.send_text(json.dumps({"type": "ready", "online": list(connections.keys())}, ensure_ascii=False))
        ...
        while True:
            ...
            if message_type == "ping":
                ...
                continue
            if message_type in {"call_offer", "call_answer", "call_ice", "call_reject", "call_busy", "call_end"}:
                try:
                    receiver_id = int(data.get("receiver_id"))
                except (TypeError, ValueError):
                    continue
                ...
                await broadcast(receiver_id, payload)
                continue
            if message_type == "typing":
                try:
                    target_id = int(data.get("receiver_id", data.get("to")))
                except (TypeError, ValueError):
                    continue
                if target_id == int(uid) or not user_by_id(target_id):
                    continue
                await broadcast(target_id, {"type": "typing", "from": uid, "sender_id": uid, "value": bool(data.get("value", False))})
                continue
            if message_type in {"message", "send_message"}:
                try:
                    ...
                    continue
                text = str(data.get("text", "") or "").strip()
                if not text or len(text) > 5000 or receiver_id == int(uid) or not user_by_id(receiver_id):
                    continue
                conn = db()
                cursor = conn.execute("INSERT INTO messages(sender_id,receiver_id,text,created_at) VALUES(?,?,?,?)", (uid, receiver_id, text, now()))
                conn.commit()
                row = conn.execute("SELECT * FROM messages WHERE id=?", (int(cursor.lastrowid),)).fetchone()
                conn.close()
                if row:
